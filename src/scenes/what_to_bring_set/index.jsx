...
const GroupsTable = ({ data }) => {
  ...
  const handleRemoveGroup = (id) => {
    ...
    const apiUrl = '/api_/classes/what-to-bring';
...
const handleShowInclusions = () => {
  ...
  console.log(">",temporaryId);
    ...
    try {
      const response = await fetch(`/api_/what-to-bring/${temporaryId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateGroup = async () => {
    ...
    console.log(">",temporaryId);
      ...
      try {
        const response = await fetch(`/api_/classes/what-to-bring/${temporaryId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
const handleRemoveInclusion = (id) => {
  // Lógica para remover uma inclusão pelo ID
  const apiUrl = '/api_/what-to-bring';
  const deleteUrl = `${apiUrl}/${id}`;
...
<Header title="What To Bring ❓" subtitle="Create and set up all the Things to bring available for the Products." />
</div>
{showAddedProductAlert && (
       <Alert sx={{marginLeft: 235, width: 300}} variant="filled" severity="success">
          Thing to Bring Added!
     </Alert>
)}
    ...
    <div className="row">
      <div className="col-md-6">
        <div className="card-body d-flex align-items-center justify-content-center">
          ...
          <h4 className="text-center" style={{ color: 'gray' }}>Add a new thing to bring to be added to the products.</h4>
          <br />
          <Box
            sx={{
              display: 'flex',
