...
const Dashboard = () => {
  ...
  const [newInclusionDescription, setNewInclusionDescription] = useState('');
  ...
  //
  const [isAccomodationTypeModalOpen, setIsAccomodationTypeModalOpen] = useState(false);
  ...
  const [selectedScenario, setSelectedScenario] = useState('accommodation'); // Definindo o estado inicial como 'accommodation' inclusions

  const toggleView = (selectedScenario) => {
...
const GroupsTable = ({ data }) => {
  ...
  const handleRemoveGroup = (id) => {
    ...
    const apiUrl = '/api_/classes/categories';
...
const AccomodationTypesTable = ({ data }) => {

  const handleRemoveAccomodationType = (id) => {
    // Lógica para remover uma inclusão pelo ID
    const apiUrl = '/api_/accomodation-types';
    const deleteUrl = `${apiUrl}/${id}`;
        ...
        <TableBody>
          {data.map((item) => (
            <TableRow key={item._id}>
              <TableCell>{item.title}</TableCell>
              <TableCell>
                <IconButton onClick={() => openAccomodationTypeModal(item)}><EditIcon /></IconButton>
                <IconButton onClick={() => handleRemoveAccomodationType(item._id)}><DeleteIcon /></IconButton>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
...
const RoomTypesTable = ({ data }) => {

  const handleRemoveRoomType = (id) => {
    // Lógica para remover uma inclusão pelo ID
    const apiUrl = '/api_/room-types';
    const deleteUrl = `${apiUrl}/${id}`;
        ...
        <TableBody>
          {data.map((item) => (
            <TableRow key={item._id}>
              <TableCell>{item.title}</TableCell>
              <TableCell>
                <IconButton onClick={() => openRoomTypeModal(item)}><EditIcon /></IconButton>
                <IconButton onClick={() => handleRemoveRoomType(item._id)}><DeleteIcon /></IconButton>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
...
const AmenitysTable = ({ data }) => {

  const handleRemoveAmenity = (id) => {
    // Lógica para remover uma inclusão pelo ID
    const apiUrl = '/api_/amenitys';
    const deleteUrl = `${apiUrl}/${id}`;
        ...
        <TableBody>
          {data.map((item) => (
            <TableRow key={item._id}>
              <TableCell>{item.title}</TableCell>
              <TableCell>
                <IconButton onClick={() => openAmenityModal(item)}><EditIcon /></IconButton>
                <IconButton onClick={() => handleRemoveAmenity(item._id)}><DeleteIcon /></IconButton>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
...
{classes.length > 0 && inclusions.length > 0 && classes.map((classObj, index) => (
  ...
  const openAccomodationTypeModal = (inclusion) => {
    // Functionality to open modal with the inclusion data
    setTemporaryId(inclusion._id);
    setNewInclusionTitle(inclusion.title);
    setNewInclusionDescription(inclusion.description);
    

    console.log("Open modal for Group:", inclusion);
    setIsAccomodationTypeModalOpen(true);
  };

  const closeAccomodationTypeModal = () => {
    setIsAccomodationTypeModalOpen(false);
  };

  const openRoomTypeModal = (inclusion) => {
    // Functionality to open modal with the inclusion data
    setTemporaryId(inclusion._id);
    setNewInclusionTitle(inclusion.title);
    setNewInclusionDescription(inclusion.description);
  ...
  const openAmenityModal = (inclusion) => {
    // Functionality to open modal with the inclusion data
    setTemporaryId(inclusion._id);
    setNewInclusionTitle(inclusion.title);
...
const handleShowInclusions = () => {
  ...
  console.log(">",temporaryId);
    ...
    try {
      const response = await fetch(`/api_/categories/${temporaryId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateGroup = async () => {
    ...
    console.log(">",temporaryId);
      ...
      try {
        const response = await fetch(`/api_/classes/categories/${temporaryId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    const updateAccomodationTypeGroup = async () => {
      ...
      console.log(">",temporaryId);
        ...
        try {
          const response = await fetch(`/api_/accomodation-types/${temporaryId}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      const updateRoomTypeGroup = async () => {
        ...
        console.log(">",temporaryId);
          ...
          try {
            const response = await fetch(`/api_/room-types/${temporaryId}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
              },
        ...
        const updateAmenityGroup = async () => {
          ...
          console.log(">",temporaryId);
            ...
            try {
              const response = await fetch(`/api_/amenitys/${temporaryId}`, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                },
...
const handleDescriptionChange = (e) => {
  // Lógica para atualizar o título enquanto o usuário digita
  // Por exemplo, você pode armazenar o título em um estado
  setNewInclusionDescription(e.target.value);
};
...
const handleAddAccomodationTypeGroup = () => {
...
<Header title="Accomodation (Configs) 🛏️" subtitle="Create and set up all the Accomodation configs available for this Product." />
</div>
{showAddedProductAlert && (
      ...
      <div className="col-md-6">
        <div className="card-body d-flex align-items-center justify-content-center">
          ...
          <Modal open={isCategoryModalOpen} onClose={closeCategoryModal}>
            ...
            <Modal open={isAccomodationTypeModalOpen} onClose={closeAccomodationTypeModal}>
              <Box sx={{ position: 'absolute', top: '50%', left: '50%', height: 400, width: 600, transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 3 }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Typography sx={{ marginRight: 2 }} variant="h5">
                           Accomodation Type 📁 | Edit
                        </Typography>
                       
                        <div style={{ marginLeft: 'auto' }}>
                        <Button onClick={closeAccomodationTypeModal} variant="contained" sx={{marginRight:'4px'}} >
                            cancel </Button>
                        <Button onClick={updateAccomodationTypeGroup} variant="contained" >
                            Save <CheckIcon sx={{marginLeft:'4px'}}/>
                        </Button>
                        </div>
                      </div>
            ...
            <Modal open={isRoomTypeModalOpen} onClose={closeRoomTypeModal}>
              <Box sx={{ position: 'absolute', top: '50%', left: '50%', height: 400, width: 600, transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 3 }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Typography sx={{ marginRight: 2 }} variant="h5">
                           Room Type 📁 | Edit
                        ...
                        <div style={{ marginLeft: 'auto' }}>
                        <Button onClick={closeRoomTypeModal} variant="contained" sx={{marginRight:'4px'}} >
                            cancel </Button>
                        <Button onClick={updateRoomTypeGroup} variant="contained" >
                            Save <CheckIcon sx={{marginLeft:'4px'}}/>
                        </Button>
                        </div>
                      </div>
            ...
            <Modal open={isAmenityModalOpen} onClose={closeAmenityModal}>
              <Box sx={{ position: 'absolute', top: '50%', left: '50%', height: 400, width: 600, transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 3 }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Typography sx={{ marginRight: 2 }} variant="h5">
                           Amenity / Feature 📁 | Edit
                        </Typography>
                       
                        <div style={{ marginLeft: 'auto' }}>
                        <Button onClick={closeAmenityModal} variant="contained" sx={{marginRight:'4px'}} >
                            cancel </Button>
                        <Button onClick={updateAmenityGroup} variant="contained" >
                            Save <CheckIcon sx={{marginLeft:'4px'}}/>
                        </Button>
                        </div>
                      </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: "2px" }}>
            <Button
              variant="contained"
              onClick={() => toggleView('accommodation')}
        ...
        {selectedScenario === 'accommodation' && (
          // Conteúdo para mostrar Accommodation Types
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <h2 className="text-center">Create a Accomodation Type</h2>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginLeft: '10px' }}></div>
          </div>
          <h4 className="text-center" style={{ color: 'gray' }}>Add a new Accomodation Type to be added to Accomodation product.</h4>
          <br />
          <Box
            sx={{
              display: 'flex',
          ...
          <div>
            <div style={{ maxHeight: '300px', overflowY: 'auto', marginTop: '20px' }}>
              <Button sx={{}} fullWidth variant="contained" color="primary" onClick={handleAddAccomodationTypeGroup}>
                Add Accomodation Type <CheckIcon />
              </Button>
              <br />
            </div>
          </div>
        ...
        {selectedScenario === 'room' && (
          // Conteúdo para mostrar Room Types
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginRight: '10px' }}></div>
            <h2 className="text-center">Create a Room Type</h2>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginLeft: '10px' }}></div>
          </div>
          <h4 className="text-center" style={{ color: 'gray' }}>Add a new Room Type to be added to Accomodation product.</h4>
          <br />
          <Box
            sx={{
              display: 'flex',
          ...
          <div>
            <div style={{ maxHeight: '300px', overflowY: 'auto', marginTop: '20px' }}>
              <Button sx={{}} fullWidth variant="contained" color="primary" onClick={handleAddRoomTypeGroup}>
                Add Room Type <CheckIcon />
              </Button>
              <br />
            </div>
          </div>
        ...
        {selectedScenario === 'amenities' && (
          // Conteúdo para mostrar Room Amenities
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginRight: '10px' }}></div>
            <h2 className="text-center">Create a Room Amenity / Feature</h2>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginLeft: '10px' }}></div>
          </div>
          <h4 className="text-center" style={{ color: 'gray' }}>Add a new Room Amenity to be added to Accomodation product.</h4>
          <br />
          <Box
            sx={{
              display: 'flex',
          ...
          <div>
            <div style={{ maxHeight: '300px', overflowY: 'auto', marginTop: '20px' }}>
              <Button sx={{}} fullWidth variant="contained" color="primary" onClick={handleAddAmenityGroup}>
                Add Room Amenity <CheckIcon />
              </Button>
              <br />
            </div>
          </div>
