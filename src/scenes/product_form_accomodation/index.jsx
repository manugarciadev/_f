...
const handleGenerateArray = () => {
  const newArray = seasons.map((season) => ({
...
const handleAddTier = (category) => {
  ...
  const fetchDataWhatToBring = async () => {
    try {
      const response = await axios.get('/api/_what-to-bring');
...
Add Amenities / Features Included  <AddIcon />
</Button>
</Box>
</Box>
</Modal>
...
<Button variant="contained" onClick={handleAddExclusions}>
  Add Amenities / Features Excluded <AddIcon />
</Button>
</Box>
</Box>
</Modal>
                 ...
                 <TableContainer style={{ height: "300px", overflowY: "auto" }} component={Paper}>
                  <Table>
                    ...
                    <TableBody>
                      {seasons.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={3}>No season added</TableCell>
                        </TableRow>
                      ) : (
                        seasons.map((season, index) => (
                          <TableRow key={index}>
