  ...
  const addLocation = async (lat, lon) => {
    try {
      ...
      if (response.ok) {
        ...
        const postResponse = await fetch('/api_/pickup-places', {
          method: 'POST',
          headers: {
...
const GroupsTable = ({ data }) => {
  ...
  const handleRemoveGroup = (id) => {
    ...
    const apiUrl = '/api_/classes/pickup-places';
...
const handleShowInclusions = () => {
  ...
  console.log(">",temporaryId);
    ...
    try {
      const response = await fetch(`/api_/pickup-places/${temporaryId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const editExecLocation = async (lat, lon) => {
    try {
        ...
        if (response.ok) {
            ...
            const putResponse = await fetch(`/api_/pickup-places/${temporaryId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
  ...
  const updateGroup = async () => {
    ...
    console.log(">",temporaryId);
      ...
      try {
        const response = await fetch(`/api_/classes/pickup-places/${temporaryId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
const handleRemoveInclusion = (id) => {
  ...
  const apiUrl = '/api_/pickup-places';
  const deleteUrl = `${apiUrl}/${id}`;
...
<Header title="Pick - up Places 🚍" subtitle="Create and set up all the Pick - up Places available for the Products." />
</div>
{showAddedProductAlert && (
      ...
      <div className="col-md-6">
        <div className="card-body d-flex align-items-center justify-content-center">
          ...
          <Modal open={isCategoryModalOpen} onClose={closeCategoryModal}>
              <Box sx={{ position: 'absolute', top: '50%', left: '50%', height: 730, width: 800, transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 3 }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        Pick - up Place 🚍 | Add Pick - up Place
                        </Typography>
            ...
            <Modal open={isEditModalOpen} onClose={closeEditModal}>
              <Box sx={{ position: 'absolute', top: '50%', left: '50%', height: 800, width: 800, transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 3 }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Typography sx={{ marginRight: 2 }} variant="h5">
                        Pick - up Place 🚍 | Edit Location
                        </Typography>
                       
                        <div style={{ marginLeft: 'auto' }}>
                        <Button onClick={closeEditModal} variant="contained" sx={{marginRight:'4px'}} >
                            cancel </Button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <h2 className="text-center">Create a New Pickup - Place</h2>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginLeft: '10px' }}></div>
          </div>
          <h4 className="text-center" style={{ color: 'gray' }}>Add a new Pickup - Place to be added to the products.</h4>
          <br />
      
          <div>
            <div style={{ maxHeight: '300px', overflowY: 'auto', marginTop: '20px' }}>
              <Button sx={{}} fullWidth variant="contained" color="primary" onClick={openCategoryModal}>
                Add Pickup - Place <CheckIcon />
              </Button>
              <br />
            </div>
          </div>
