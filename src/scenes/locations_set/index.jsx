  ...
  const addLocation = async (lat, lon) => {
    try {
      ...
      if (response.ok) {
        ...
        const postResponse = await fetch('/api_/locations', {
          method: 'POST',
          headers: {
...
const GroupsTable = ({ data }) => {
  ...
  const handleRemoveGroup = (id) => {
    ...
    const apiUrl = '/api_/classes/locations';
...
const handleShowInclusions = () => {
  ...
  console.log(">",temporaryId);
    ...
    try {
      const response = await fetch(`/api_/locations/${temporaryId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const editExecLocation = async (lat, lon) => {
    try {
        ...
        if (response.ok) {
            ...
            const putResponse = await fetch(`/api_/locations/${temporaryId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
  ...
  const updateGroup = async () => {
    ...
    console.log(">",temporaryId);
      ...
      try {
        const response = await fetch(`/api_/classes/locations/${temporaryId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
<Header title="Destinations 🗺️" subtitle="Create and set up all the Destinations available for the Products." />
</div>
{showAddedProductAlert && (
      ...
      <div className="col-md-6">
        <div className="card-body d-flex align-items-center justify-content-center">
          ...
          <Modal open={isCategoryModalOpen} onClose={closeCategoryModal}>
              <Box sx={{ position: 'absolute', top: '50%', left: '50%', height: 730, width: 800, transform: 'translate(-50%, -50%)', bgcolor: 'white', p: 3 }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        Destinations 🗺️ | Add Destination
                        </Typography>
                       
                        <div style={{ marginLeft: 'auto' }}>
                        <Button onClick={closeCategoryModal} variant="contained" sx={{marginRight:'4px'}} >
                            cancel </Button>
                        <Button onClick={() => addLocation(markerPosition.lat, markerPosition.lng)} variant="contained" >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <h2 className="text-center">Create a New Destination</h2>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginLeft: '10px' }}></div>
          </div>
          <h4 className="text-center" style={{ color: 'gray' }}>Add a new Destination to be added to the products.</h4>
          <br />
      
          <div>
            <div style={{ maxHeight: '300px', overflowY: 'auto', marginTop: '20px' }}>
              <Button sx={{}} fullWidth variant="contained" color="primary" onClick={openCategoryModal}>
                Add Destinations <CheckIcon />
              </Button>
              <br />
            </div>
          </div>
