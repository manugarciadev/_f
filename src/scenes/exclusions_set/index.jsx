...
const GroupsTable = ({ data }) => {
  ...
  const handleRemoveGroup = (id) => {
    ...
    const apiUrl = '/api_/classes/exclusions';
...
const toggleLanguageSelection = (targetLang, targetText) => {
  ...
  console.log(">",temporaryId);
    ...
    try {
      const response = await fetch(`/api_/exclusions/${temporaryId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateGroup = async () => {
    ...
    console.log(">",temporaryId);
      ...
      try {
        const response = await fetch(`/api_/classes/exclusions/${temporaryId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
<Header title="Exclusions ❎" subtitle="Create and set up all the Exclusions available for the Products." />
</div>
{showAddedProductAlert && (
