...
{classes.length > 0 && inclusions.length > 0 && classes.map((classObj, index) => (
  ...
  const openCategoryModal = (inclusion) => {
    ...
    setStartAge(inclusion.startAge);
    setStartAge(inclusion.endAge);
...
const handleShowInclusions = () => {
  ...
  console.log(">",temporaryId);
    ...
    try {
      const response = await fetch(`/api_/age-ranges/${temporaryId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateGroup = async () => {
    ...
    console.log(">",temporaryId);
      ...
      try {
        const response = await fetch(`/api_/classes/age-ranges/${temporaryId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
<Header title="Age Ranges 🔢" subtitle="Create and set up all the Age Ranges available for the Products." />
</div>
{showAddedProductAlert && (
      ...
      <div className="col-md-6">
        <div className="card-body d-flex align-items-center justify-content-center">
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <h2 className="text-center">Create a New Age Range</h2>
            <div style={{ flex: 1, borderBottom: '1px solid black', marginLeft: '10px' }}></div>
          </div>
          <h4 className="text-center" style={{ color: 'gray' }}>Add a new Age Range to be added to the products.</h4>
          <br />
          <Box
            sx={{
              display: 'flex',
